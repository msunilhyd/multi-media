...
async def fetch_fifa_highlights(background_tasks: BackgroundTasks) -> Dict[str, Any]:
    ...
    async def fetch_fifa_highlights_task():
        try:
            ...
            # FOX Sports YouTube channel ID
            FOX_SPORTS_CHANNEL_ID = "UCwNqHDsnBCKT-olwJwIFyfg"
            ...
            for match in matches_needing_highlights:
                try:
                    # Use search().list() with channelId to search WITHIN FOX Sports channel only
                    # search().list() supports q= parameter - costs 100 units but scoped to 1 channel
                    query = f"{match.home_team} vs {match.away_team} Extended Highlights"
                    print(f"[FIFA] Searching FOX Sports channel for: {query}")
                    
                    response = youtube_service.youtube.search().list(
                        part='snippet',
                        ...
                        if home_in_title and away_in_title and has_extended:
                            ...
                            if not existing:
                                new_highlight = models.Highlight(
                                    ...
                                    channel_title=snippet.get('channelTitle', 'FOX Sports'),
                                    published_at=snippet.get('publishedAt')
                                )
                                db.add(new_highlight)
                                videos_added += 1
                                highlights_found += 1
                                print(f"[FIFA] ✓ Added from FOX Sports: {title[:70]}")
...
async def add_fifa_world_cup_matches(db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    # All FIFA World Cup 2026 matches - dates verified from official Google/FIFA schedule
    fifa_matches = [
        # GROUP STAGE - Matchday 1 (Jun 11-12) - FINISHED
        ...
        # GROUP STAGE - Matchday 2 (Jun 13-15) - FINISHED
        ...
        # GROUP STAGE - Matchday 3 (Jun 16-19) - FINISHED
        ...
        {"home": "Germany", "away": "Curaçao", "date": date(2026, 6, 16), "status": "finished"},
        ...
        # GROUP STAGE - Matchday 4 (Jun 20) - FINISHED
        {"home": "Germany", "away": "Ivory Coast", "date": date(2026, 6, 20), "status": "finished"},
        ...
        # GROUP STAGE - Matchday 5 (Jun 21) - FINISHED
        {"home": "Spain", "away": "Saudi Arabia", "date": date(2026, 6, 21), "status": "finished"},
        {"home": "Belgium", "away": "Iran", "date": date(2026, 6, 21), "status": "finished"},
        {"home": "Uruguay", "away": "Cabo Verde", "date": date(2026, 6, 21), "status": "finished"},
        {"home": "New Zealand", "away": "Egypt", "date": date(2026, 6, 21), "status": "finished"},
        {"home": "Scotland", "away": "Brazil", "date": date(2026, 6, 21), "status": "finished"},
        {"home": "Morocco", "away": "Haiti", "date": date(2026, 6, 21), "status": "finished"},
        {"home": "Türkiye", "away": "Paraguay", "date": date(2026, 6, 21), "status": "finished"},
        {"home": "USA", "away": "Australia", "date": date(2026, 6, 21), "status": "finished"},
        # GROUP STAGE - Matchday 6 (Jun 22) - TODAY
        ...
        # GROUP STAGE - Matchday 9 (Jun 25)
        {"home": "Curaçao", "away": "Ivory Coast", "date": date(2026, 6, 25), "status": "scheduled"},
        ...
        # GROUP STAGE - Matchday 10 (Jun 26)
        ...
        # GROUP STAGE - Matchday 11 (Jun 27)
