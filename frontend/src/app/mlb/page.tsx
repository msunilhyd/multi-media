      ...
      for (const leagueGroup of highlightsData) {
        for (const match of leagueGroup.matches) {
          if (match.highlights && match.highlights.length > 0) {
            ...
            console.log('⚾ [MLB] Auto-loaded first available video:', match.home_team, 'vs', match.away_team);
            return;
          }
        }
      }
    ...
    try {
      ...
      if (league || (teams && teams.length > 0)) {
        ...
        if (!seasonStartStr) {
          ...
          const seasonStartYear = currentMonth < 3 ? currentYear - 1 : currentYear;
      ...
      const data = await fetchHighlightsGroupedByDate(date, 'mlb');
      ...
      <main className="container mx-auto px-3 sm:px-4 py-6 sm:py-8">
        <div className="text-center mb-8">
          ...
          <p className="text-base sm:text-lg text-gray-600 dark:text-gray-400 max-w-3xl mx-auto mb-4">
            Watch free MLB game highlights from all teams. Regular season, playoffs, and World Series coverage. Updated daily with the latest match videos and replays.
          </p>
          <p className="text-sm sm:text-base text-gray-500 dark:text-gray-500">
            Free highlights • All MLB teams • Daily updates • Full HD quality • No subscription required
          </p>
        </div>
