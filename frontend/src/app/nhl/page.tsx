...
export default function NHLPage() {
  ...
  useEffect(() => {
    const loadHighlights = async () => {
      try {
        ...
        const data = await fetchHighlightsGroupedByDate(today, 'nhl');
        setHighlights(data);
        setError(null);
      } catch (err) {
        console.error('Error loading NHL highlights:', err);
        setError('Failed to load NHL highlights. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDateChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setLoading(true);
      const data = await fetchHighlightsGroupedByDate(newDate, 'nhl');
      setHighlights(data);
      setError(null);
    } catch (err) {
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-4xl font-bold text-white">NHL Highlights</h1>
          </div>
          <p className="text-slate-400">Watch the latest NHL hockey game highlights from House of Highlights</p>
        </div>
        ...
        {loading && (
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="text-slate-400">Loading NHL highlights...</p>
            </div>
          </div>
        )}
        ...
        {!loading && highlights.length > 0 ? (
          ...
          <div className="text-center py-12">
            <p className="text-slate-400 text-lg">No NHL highlights available for this date</p>
          </div>
        ) : null}
      </main>
