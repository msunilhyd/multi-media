...
export default function TennisPage() {
  ...
  useEffect(() => {
    const loadHighlights = async () => {
      try {
        ...
        const data = await fetchHighlightsGroupedByDate(today, 'tennis');
        setHighlights(data);
        setError(null);
      } catch (err) {
        console.error('Error loading Tennis highlights:', err);
        setError('Failed to load Tennis highlights. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDateChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setLoading(true);
      const data = await fetchHighlightsGroupedByDate(newDate, 'tennis');
      setHighlights(data);
      setError(null);
    } catch (err) {
