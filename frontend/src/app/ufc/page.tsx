...
export default function UFCPage() {
  ...
  const loadHighlights = async (date: string) => {
    try {
      setLoading(true);
      const data = await fetchHighlightsGroupedByDate(date, 'ufc');
      setHighlights(data);
      setError(null);
    } catch (err) {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-4xl font-bold text-white">UFC</h1>
              <p className="text-slate-400 mt-1">Ultimate Fighting Championship Highlights</p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <input
                ...
                className="px-4 py-2 bg-slate-700 text-white rounded-lg border border-slate-600 focus:border-red-500 focus:outline-none"
              />
              <button
