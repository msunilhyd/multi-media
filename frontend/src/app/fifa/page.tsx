...
export default function FIFAPage() {
  ...
  useEffect(() => {
    const loadHighlights = async () => {
      try {
        ...
        const data = await fetchHighlightsGroupedByDate(today, 'fifa');
        setHighlights(data);
        setError(null);
      } catch (err) {
        console.error('Error loading FIFA highlights:', err);
        setError('Failed to load FIFA World Cup highlights. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDateChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setLoading(true);
      const data = await fetchHighlightsGroupedByDate(newDate, 'fifa');
      setHighlights(data);
      setError(null);
    } catch (err) {
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-4xl font-bold text-white">FIFA World Cup</h1>
          </div>
          <p className="text-slate-400">Watch the latest FIFA World Cup match highlights</p>
        </div>
